    ...
    private func connectionPanel(profile: Binding<ServerProfile>) -> some View {
        DashboardPanel {
            VStack(alignment: .leading, spacing: DashboardLayout.panelSpacing) {
                ...
                Text("Controller URL 建议填写 API 根地址，例如 http://192.168.9.202:9090；也可直接粘贴部署结果里的 /ui/、/Ui/ 地址，保存时会去掉 UI 段并保留反代前置路径。代码不会自动补 9090；非 80/443 端口必须显式填写。")
    ...
    private func corePanel(profile: Binding<ServerProfile>) -> some View {
        DashboardPanel {
            VStack(alignment: .leading, spacing: DashboardLayout.panelSpacing) {
                ...
                Text("Management URL 不是 Mihomo Controller。它位于最后兜底：Core API / mihomo.service 不可用时处理生命周期与日志，并继续承担订阅写入、配置生成和整套项目升级。可直接粘贴 /api/status 等部署结果地址，保存时会回退到面板根路径并保留反代前置路径；不会自动补 29090。")
                    .font(.system(size: 10.5))
                    .foregroundStyle(DashboardPalette.tertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    var body: some View {
        Form {
            Section("Mihomo Core 连接") {
                ...
                HStack {
                    Text("可填 API 根地址，也可粘贴 /ui/、/Ui/ 页面地址；保存时自动去掉 UI 段、保留反代前置路径。端口不会自动补，非 80/443 时请显式填写（Mihomo 常见为 9090）。")
                        .font(.caption)
...
private struct GeneralSettingsView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("安全") {
                Text("Controller Secret 对应 Mihomo config.yaml 的 secret；可选 Management Secret 仅用于扩展管理功能。两者均使用 macOS Keychain；Controller Secret 留空时保留兼容复用行为。Controller/Management URL 都不会自动补端口；/ui/（含大小写变体）及已知 Management API 后缀会自动归一化，同时保留反代前置路径。原生 URLSession 不受浏览器 CORS 限制，同时保留系统 TLS 证书验证。")
                    .foregroundStyle(.secondary)
            }
