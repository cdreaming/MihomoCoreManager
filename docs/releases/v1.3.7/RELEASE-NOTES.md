...
- Core 控制页同版本继续重构：第一行保持 `服务信息 | 服务控制 = 2:3`，`MetaCubeXD` 整合进“服务控制”；下方整行组合卡按 `Core 端口设置` → `项目升级` 上下排列。
...
- SwiftUI：MetaCubeXD 并入服务控制；端口行“保存”按钮固定为 88×38pt；项目升级并入 Core 端口设置卡并置于端口区下方。
- GoWebUI：同步相同模块合并结构，并改为固定宽度的逐端口“保存”按钮；端口 API 与线路端口页面保持不变。
