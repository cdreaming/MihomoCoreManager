...
## GitHub Release API 5xx 发布恢复

- 修复 Release workflow 将所有 `gh release view` 非零退出都误判为“Release 不存在”的问题。GitHub API 500/502/503/504 或网络故障现在只进入有界重试，只有明确 HTTP 404 才允许执行 `gh release create`。
- `gh release create` 失败后先重新探测 release，再决定是否重试 create；覆盖“服务端已创建成功、客户端却收到 5xx/断线”的半成功状态，避免盲目重复创建。
- Release title/notes 更新、逐资产 `--clobber` 上传及在线回读下载均加入指数退避重试；每个资产单独上传，单项失败不会重新上传全部文件。
- 增加 `scripts/test-github-release.sh`，本地模拟“已有 release / 真 404 / 先 500 后 404 / create 半成功 / upload 503”五类场景，并作为统一 release simulation 的门禁。
- 新增 `Retry Existing GitHub Release` 手工 workflow：用于 `v1.3.1` 这类 tag 已经存在但 Release API 阶段失败的情况。它从当前分支取得修复后的发布工具，但构建源码严格 checkout 原有 tag，不移动、不重建 tag。
- Release failure diagnostics 移到发布链末尾，因此创建、上传或在线验证阶段失败时同样能收集 `dist/`、portable 和 Xcode 日志。
