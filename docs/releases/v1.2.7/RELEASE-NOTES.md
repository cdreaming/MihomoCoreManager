...
- canonical simulation 现在在任何真实 checkout 上先执行 manifest `--write -> --check`，再进入 preflight；提交的 manifest 过期时只告警并自动刷新，不再让新增历史文档挡在 Xcode/Go 门禁之前。
- 恢复并纳入 `QA-v1.2.6.md`、`RELEASE-HARDENING-v1.2.6.md`，避免从 v1.2.6 仓库升级 v1.2.7 时出现“仓库文件存在但源码包 manifest 缺失”的漂移。
