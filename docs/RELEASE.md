...
v1.2.10 以 v1.2.9 为功能基线，并保留 v1.2.5 fixed5 起已验证的发布硬门禁。不要把 CI、Release、preflight 拆成彼此不同的检查链路。

1. 确认 `VERSION=1.2.10`、Xcode `MARKETING_VERSION=1.2.10`、`CURRENT_PROJECT_VERSION=1210`。
...
9. GitHub `.pkg` 重点回归：状态栏图标/状态/双行网速可按开关显示；代理组显示当前选择；菜单背景/滚动行为符合 v1.2.10；主窗口右侧背景不再近纯黑。
10. CI 与正式 Release 都只调用 `scripts/simulate-release.sh`。
11. 推送新 tag `v1.2.10`，或在 `macOS Release` workflow 中输入 `1.2.10`。不要移动已经存在且指向旧 commit 的 tag。
