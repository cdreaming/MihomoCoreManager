...
v1.2.9 以 v1.2.7 稳定发布工程为基线，并保留 v1.2.5 fixed5 起已验证的发布硬门禁。不要把 CI、Release、preflight 拆成彼此不同的检查链路。

1. 确认 `VERSION=1.2.9`、Xcode `MARKETING_VERSION=1.2.9`、`CURRENT_PROJECT_VERSION=129`。
...
7. strict macOS semantic probe 必须通过 `xcrun --sdk macosx swiftc`、当前 Xcode SDK 与 `arm64-apple-macos14.0`，禁止恢复裸 `swiftc -typecheck`。
...
11. 推送新 tag `v1.2.9`，或在 `macOS Release` workflow 中输入 `1.2.9`。不要移动已经存在且指向旧 commit 的 `v1.2.9` tag。
