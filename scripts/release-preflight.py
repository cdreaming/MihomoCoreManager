...
def main() -> int:
    ...
    # SOURCE-SHA256SUMS.txt is generated metadata.  The v1.2.5 successful
    # release path rebuilt it from the exact checkout before strict macOS gates.
    # Keep that behavior self-contained here as well, so workflow refactors cannot
    # reintroduce a stale-manifest blocker ahead of Xcode.
    if args.strict_macos:
        ...
        after = manifest.read_bytes() if manifest.exists() else b""
        if before != after:
    ...
    # v1.2.5 release guard: every Swift file on disk must be referenced by the
    # Xcode project and included in the Sources build phase.  Syntax parsing a
    # forgotten file is not enough; Xcode must actually compile it.
    pbx = (ROOT / "MihomoCoreManager.xcodeproj/project.pbxproj").read_text(encoding="utf-8")
    sources = swift_files()
    if not sources:
        raise SystemExit("FAIL: no Swift sources found")
    for source in sources:
        rel = source.relative_to(ROOT).as_posix()
        if rel not in pbx:
            raise SystemExit(f"FAIL: Swift source is not referenced by Xcode project: {rel}")
        if f"{source.name} in Sources" not in pbx:
            raise SystemExit(f"FAIL: Swift source is not in PBXSourcesBuildPhase: {rel}")
