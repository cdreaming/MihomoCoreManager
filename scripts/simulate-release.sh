...
(
  cd portable-runtime
  # v1.2.5 proved that test-owned background work and TempDir cleanup must be
  # deterministic on macOS/APFS. Keep temp files inside the checkout and run
  # both targeted and shuffled stress with explicit timeouts.
  export TMPDIR="$ROOT/build/simulated-test-tmp"
  rm -rf "$TMPDIR"
  mkdir -p "$TMPDIR"
  CGO_ENABLED=0 go test -timeout=90s ./...
  CGO_ENABLED=0 go test -timeout=120s -run '^TestGroupDelayCacheDecoratesSharedProxyData$' -count=300 ./...
  CGO_ENABLED=0 go test -timeout=180s -shuffle=on -count=10 ./...
