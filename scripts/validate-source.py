...
for marker in ["Install-MihomoManager.command", "codesign --force --deep --sign -", "codesign --verify --deep --strict", "xattr -dr com.apple.quarantine", "scripts/build-gowebui-app.sh"]:
    if marker not in portable_installer:
...
if "grid-template-columns:minmax(0,2fr) minmax(0,3fr)" not in portable_ui:
    errors.append("portable v1.3.7 Core layout must use the requested 2:3 column ratio")
...
meta_cube_index = portable_ui.find("<h3>MetaCubeXD</h3>")
project_update_index = portable_ui.find("<h3>项目升级</h3>")
if meta_cube_index < 0 or project_update_index < 0 or meta_cube_index > project_update_index:
    errors.append("portable v1.3.7 Core layout must place MetaCubeXD before 项目升级")
if "WeightedPairLayout" not in core_view or "leadingWeight: 2" not in core_view or "trailingWeight: 3" not in core_view:
    errors.append("native v1.3.7 Core layout must use the requested 2:3 weighted columns")
...
for marker in ["corePortPanel", "lowerColumns", "LazyVGrid", "minHeight: 574"]:
...
release_retry_workflow = (root / ".github/workflows/release-retry.yml").read_text(encoding="utf-8")
ci_workflow = (root / ".github/workflows/ci.yml").read_text(encoding="utf-8")
github_release_script = (root / "scripts/github-release.sh").read_text(encoding="utf-8")
github_release_tests = (root / "scripts/test-github-release.sh").read_text(encoding="utf-8")
...
for marker in [
    ...
    "WARNING: committed SOURCE-SHA256SUMS.txt was stale; refreshed from this exact checkout.",
    "bash scripts/test-github-release.sh",
    "-count=100",
    "-shuffle=on -count=3",
...
manifest_write_pos = simulate_release.find("python3 scripts/build-source-manifest.py --write")
manifest_check_pos = simulate_release.find("python3 scripts/build-source-manifest.py --check")
...
if workflow.count("bash scripts/simulate-release.sh") != 1 or ci_workflow.count("bash scripts/simulate-release.sh") != 1:
    errors.append("CI and Release must each invoke the same canonical release simulation exactly once")
...
for marker in [
    ...
    'bash scripts/github-release.sh download "$VERIFY"',
...
for marker in [
    ...
    "HTTP 404",
    "transient/unknown",
    "will NOT call create until a real 404 is observed",
    "gh release create",
    "gh release upload",
    "--clobber",
    "gh release download",
    ...
    if marker not in github_release_script:
        errors.append(f"v1.3.1 GitHub Release resilience gate missing: {marker}")
for marker in [
    "probe500-then-missing",
    "create-ambiguous",
    "upload-transient",
    "create happened before a confirmed 404",
    "expected exactly one create",
]:
    if marker not in github_release_tests:
        errors.append(f"v1.3.1 GitHub Release retry test gate missing: {marker}")
for marker in [
    "Retry Existing GitHub Release",
    "Checkout exact existing release tag",
    "github-release.sh",
    "不会移动或重建 tag",
    "Existing-tag GitHub Release verification: PASS",
]:
    if marker not in release_retry_workflow:
        errors.append(f"v1.3.1 existing-tag release recovery gate missing: {marker}")
