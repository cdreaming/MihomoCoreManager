...
4. **不猜业务端口。** Controller、Management、MetaCubeXD 均保留用户配置 URL 的显式端口；只有 SSH 默认 22。
5. **GoWebUI 的端口容错不等于改端口。** 直连 Controller/Management 发生无路由、拒绝连接、超时或临时网关故障时，若存在可用 SSH 路径，会在服务器侧先请求原 URL；若服务器自身也无法访问该地址，再把同一 scheme/port/path 改为 `127.0.0.1` 重试。
...
### 2.3 GoWebUI：Controller HTTP-over-SSH 传输容错

Controller 的**接口语义不变**，仍然是 Mihomo Core API。GoWebUI 仅在客户端无法直接到达配置端口时切换传输路径：
...
- `systemdSSHTarget`：例如 `root@192.168.1.2`。**GoWebUI 留空时会从 LAN `managementURL`，其次从 LAN `coreControllerURL` 自动推断 host**；如服务器 SSH 用户名不同，应显式填写 `user@host`。SwiftUI 当前仍以显式填写为准。
- `systemdSSHPort`：SSH 端口，默认 `22`，允许 `1...65535`。
- `systemdIdentityFile`：可选私钥路径；留空时使用系统 ssh-agent / SSH 默认行为。

旧 v1.3.1 Profile 不含这些字段也能正常解码。GoWebUI 对 RFC1918/localhost 风格 LAN Profile 可自动推断 SSH 主机；公共域名不会被自动探测 22 端口。
...
若 SSH / journal 均失败且已配置 Management，则回退 `/api/logs?lines=N`。GoWebUI 对这个 Management 请求同样应用 HTTP-over-SSH 本机端口容错；若两层都失败，错误信息会同时保留 journal/systemd 与面板失败原因。
...
GoWebUI 对 Management 接口也有与 Controller 相同的**传输容错**：客户端直连失败后，经 SSH 在服务器侧请求原 URL；连接仍失败时再请求 `127.0.0.1:<Management 原端口>`。因此 `/api/subscriptions`、`/api/logs`、`/api/project-update/check|apply|log` 即使只绑定 localhost，也不要求把 29090 暴露到客户端 LAN。
...
| `mihomo.service` SSH | `systemdSSHPort` | **22** | GoWebUI 可从 LAN Management/Controller host 推断 SSH 主机；端口仍固定来自此字段 |
