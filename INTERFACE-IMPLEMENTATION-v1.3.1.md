# Mihomo Core Manager v1.3.1 接口与端口实现说明

> 适用版本：**Mihomo Core Manager v1.3.1 / build 1301**。
>
...
|---|---|---|---|---|
| Core 在线状态 / 版本 / 连接统计 | Mihomo Controller | `mihomo.service` 状态 | Core 服务面板 `/api/status` | Controller 在线时信息最接近 Core 本体；Core 已停止时改由 systemd 判断 |
...
| 重载 `config.yaml` | Mihomo `PUT /configs?force=true` | `systemctl reload mihomo.service` | Core 服务面板 `/api/action` | 先使用官方运行配置 API |
| 最近 N 行服务日志 | `journalctl -u mihomo.service` | Core 服务面板 `/api/logs` | 无 | Mihomo `/logs` 是实时流，不等价于历史 journal |
| 运行模式读取/切换 | Mihomo `/configs` | 无 | 无 | 纯 Core 运行态能力 |
| 代理列表 / 策略组 / provider | Mihomo `/proxies`、`/group`、`/providers/proxies` | 无 | 无 | 纯 Core API |
| 代理选择 / 策略组测速 | Mihomo `/proxies/{group}`、`/group/{group}/delay` | 无 | 无 | 测速目标 URL 由 Core 访问，客户端只访问 Controller |
| 订阅读取/写入与配置渲染事务 | Core 服务面板 `/api/subscriptions` | 无 | 无 | 当前产品语义包含订阅源持久化、渲染、校验、回滚；不是 Mihomo provider refresh 的等价物 |
| 项目升级（Core + MetaCubeXD + 面板） | Core 服务面板 `/api/project-update/*` | 无 | 无 | Mihomo `/upgrade` 只覆盖 Core，不能原样替代整套项目升级 |
| 打开 MetaCubeXD | Profile 中的 MetaCubeXD URL | 服务面板状态返回的 URL（兼容） | 无 | 只是打开 Dashboard URL，不是 Core 控制协议 |
...
4. **不猜端口。** Controller、Management、MetaCubeXD 均保留用户配置 URL 的显式端口；只有 SSH 在启用 `mihomo.service` 管理时默认 22。
...
- `Controller Secret`：优先使用独立 Controller Secret；为空时保留 v1.3.0 兼容行为，复用 Management Secret。
- 请求头：`Authorization: Bearer <secret>`（Secret 非空时）。
- `coreControllerURL` 如果误填 `/ui/`，客户端会规范化到 API 根地址。
- **不会自动补 `:9090`。** URL 未写端口时按 URL 标准使用 HTTP 80 / HTTPS 443；非默认端口必须由用户显式写入。
- 明文 `http://` 仍受 Profile 的“允许不安全 HTTP”设置约束。
...
| `PUT` | `/configs?force=true` | 以 `{path, payload}` 重载 Profile 配置文件 | `systemctl reload` → 面板 reload |
...
- `systemdSSHTarget`：例如 `root@192.168.1.2`；留空则禁用 systemd 直连。
- `systemdSSHPort`：SSH 端口，默认 `22`，允许 `1...65535`。
- `systemdIdentityFile`：可选私钥路径；留空时使用系统 ssh-agent / SSH 默认行为。

旧 v1.3.1 Profile 不含这些字段也能正常解码，默认保持原行为。

### 3.2 SSH 安全与非交互参数

两套实现（SwiftUI / GoWebUI）使用相同策略：
...
```bash
systemctl start|stop|restart|reload mihomo.service
```

非 root 用户：

```bash
sudo -n systemctl start|stop|restart|reload mihomo.service
```
...
```bash
journalctl -u mihomo.service -n <10...300> --no-pager -o cat
```

若普通用户无 journal 读取权限，再尝试：

```bash
sudo -n journalctl -u mihomo.service -n <10...300> --no-pager -o cat
```

若 SSH / journal 均失败且已配置 Management，则回退 `/api/logs?lines=N`。
...
**不会自动补 `:29090`。** 示例端口只出现在 UI placeholder/文档中，实际网络请求完全使用用户填写的 Management URL。
...
| `mihomo.service` SSH | `systemdSSHPort` | **22** | 唯一由 App 明确提供的远端端口默认值；可改 1...65535 |
...
- `9090` 仅存在于 Controller URL 示例、测试和“不自动补 9090”的说明。
- `29090` 仅存在于 Management URL 示例。
...
| 本地接口 | 作用 | 远端实现 |
|---|---|---|
| `/local/status` | 状态快照 | Controller → systemd → Management |
| `/local/action` | 生命周期动作 | restart: Core → systemd → Management；start/stop: systemd → Management |
| `/local/reload-config` | 配置重载 | Core `/configs` → systemd reload → Management |
| `/local/logs` | 最近 N 行日志 | journal → Management |
| `/local/proxy-mode` | 运行模式 | Core `/configs` |
| `/local/proxy-groups` | 策略组 | Core `/group` + `/proxies` |
| `/local/proxies` | 代理快照 | Core `/proxies` + `/providers/proxies` |
| `/local/proxy-delay*` | 测速 | Core `/group/{name}/delay` |
| `/local/proxy-select*` | 代理选择 | Core `/proxies/{name}` |
| `/local/subscriptions` | 订阅事务 | Management `/api/subscriptions` |
| `/local/update/*` | 项目升级 | Management `/api/project-update/*` |
...
- `portable-runtime/main.go`：`systemdSSHArgs`、`runSystemdSSH`、`systemdStatusSnapshot`、`systemdLifecycleAction`、`systemdLogs` 以及各 `/local/*` handler。
- `portable-runtime/ui/index.html`：systemd SSH 配置、unit path 展示、后端优先级提示。
- `portable-runtime/main_test.go`：优先级与回退的回归测试。

## 8. 失败与容错行为

- Controller 失败**不会**立即把整个后端判死：状态继续尝试 systemd，再尝试 Management。
- `restart` / `reload` 的 Core API 失败后继续 systemd，再到 Management。
- `start` / `stop` 不先探测 Controller，因为它不是对应生命周期能力；直接 systemd，再到 Management。
- systemd SSH 如果主机不可达、SSH 认证失败、没有 `mihomo.service`、`sudo -n` 无权限或命令失败，会进入该操作允许的下一级回退。
...
- reload 必须 `Controller -> systemd -> Management`。
- status 必须 `Controller -> systemd -> Management`。
- logs 必须 `journal -> Management`。
