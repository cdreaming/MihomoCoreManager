...
## Xcode SDK-bound Swift preflight follow-up

A real Xcode 16.4 run exposed a preflight-command bug: standalone
`swiftc -typecheck` was invoked without the macOS SDK and failed with
`unable to load standard library`. The semantic probe now resolves the active
macOS SDK via `xcrun`, passes it explicitly, and targets
`arm64-apple-macos14.0`, matching the project deployment target.

## Portable runtime macOS cleanup-race follow-up

A real macOS arm64 Go test exposed that `runProxyDelay` launched a proxy-menu
cache refresh after the synchronous handler had already returned. The refresh
could create `Runtime/proxies.json` while `testing.TempDir` was removing the
same directory, producing `TempDir RemoveAll cleanup: directory not empty`.

All app-owned short-lived asynchronous refresh work is now tracked by a
`sync.WaitGroup`, affected tests wait for background work in `t.Cleanup`, and
the release preflight stress-runs the exact regression 100 times on macOS,
shuffles the full suite, runs the Go race detector, runs `go vet`, and
cross-compiles the darwin/arm64 runtime and test binary before Xcode packaging.

## Canonical real-runner simulation

To avoid fixing release failures one gate at a time, both CI and Release now run
one canonical `scripts/simulate-release.sh` on GitHub `macos-15` arm64. The
same script exercises portable-runtime stress/race tests and cross-compilation,
builds/verifies the portable installer, then runs strict Xcode preflight and the
actual unsigned native Release build before any GitHub Release upload occurs.
