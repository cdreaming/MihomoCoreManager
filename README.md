...
基于 **Mihomo Core 管理面板 v4.0.0** API 开发的 Apple Silicon（arm64）macOS 管理客户端。当前 App 版本为 **v1.3.1 (build 1301)**；`v4.0.0` 是服务端兼容基线，不是 App 版本。

## v1.3.1

> **同版本修订：** 本源码仍是 `v1.3.1 / build 1301`，不升级版本。主窗口左侧后端选择标签修正为“后端管理”；“服务设置”补回 v1.3.0 的刷新间隔/默认日志行数等能力；远端控制进一步按 **Mihomo Core API → `mihomo.service` / systemd → Core 服务面板兜底** 的能力边界执行。完整接口巡查见 `API-AUDIT-v1.3.1.md`，详细接口/端口实现见 `INTERFACE-IMPLEMENTATION-v1.3.1.md`。
