...
- Core 在线数据与 Core 内部控制：优先 **Mihomo Controller API**；Core 面板 / MetaCubeXD 版本只从 v4.0.1 Management `/api/status` 以慢速元数据通道补齐，不再误把 Controller 的 core-only 结果当成完整三组件版本。
- Linux 服务生命周期：优先服务器现有 **`mihomo.service` / systemd**。
- Core 服务面板：作为生命周期/日志的**备选与容错**，并继续承担没有 Core/systemd 等价实现的订阅事务与整套项目升级；Controller/Management Secret 任一未单独设置时双向复用另一个。
...
| 功能 | 优先级 |
|---|---|
| 状态 | Controller `/version` + `/connections` → `systemctl show mihomo.service` → `/api/status`；Controller/systemd 成功时每 30 秒补一次 Management 版本/部署元数据 |
...
| 配置重载 | Controller `PUT /configs?force=true` → `systemctl reload mihomo.service`（仅 `CanReload=yes`）→ `/api/action` |
...
| 三组件项目升级 | Core 服务面板 `/api/project-update/*`；直连失败时 Management over SSH；升级完成后清理版本元数据缓存并重新读取 |
...
| config.yaml 热重载 | `PUT /configs?force=true` + `{path,payload}` | **Core → systemd reload（仅 CanReload=yes）→ 面板** |
...
- 重载：先 `systemctl show ... --property=CanReload --value`，仅返回 `yes` 时才执行 `systemctl reload mihomo.service`；v4.0.1 默认 unit 无 ExecReload 时直接跳到 Management fallback
- 日志：`journalctl -u mihomo.service -n N --no-pager -o short-iso`

非 root SSH 用户的修改型操作使用 `sudo -n`；失败自动进入面板回退。状态解析使用 `systemctl show` 的属性输出，不解析面向人的 `systemctl status` 文本。为对齐 v4.0.1 的启动策略，direct systemd 的 `start/restart` 会先 best-effort `disable mihomo.service`，避免 Manager 意外把手工运行语义变成开机自启。
...
- **MetaCubeXD：** 不猜测端口。显式 LAN URL 优先；否则当 Management 是 LAN 且 `/api/status` 报告 standalone port 时使用同服务器 LAN host + 该端口；再回退显式/状态公网 URL。
...
- `/api/status`：Controller + systemd 均失败后的状态回退；在 Controller/systemd 成功时也作为 30 秒慢速元数据源补齐 `versions.management_panel`、`versions.metacubexd` 和部署信息，且不得覆盖 Controller Core 版本。
...
- `/local/status` → Controller → systemd → Management；正常 Controller/systemd 快照再合并慢速 Management 元数据
- `/local/action` → restart 时 Core → systemd → Management；start/stop 时 systemd → Management
- `/local/reload-config` → Core `/configs` → systemd reload（CanReload=yes 才执行）→ Management
...
- `TestControllerStatusEnrichesPanelVersionsAndSharesControllerSecret`
- `TestDeploymentAwareURLNormalizationKeepsPortsPrefixesAndSetup`
- `TestResolvedMetaCubeXDUsesReportedLANPortBeforePublicURLWithoutGuessing`
- `TestManagementSecretFallsBackSymmetrically`
- `TestEndpointNormalizationRejectsWildcardListenerAddresses`
- `TestReloadSkipsUnsupportedSystemdAndFallsBackToManagement`
...
本轮以用户提供的 `mihomo-web-installer-v4.0.1-chatgpt-config-persistence-fixed-source.zip` **实际源码**为对照，而不是仅按旧版文档推断。关键结果如下：
...
### Bug ①：侧栏 “Core 面板 / MetaCubeXD” 版本长期为 `--` 或错误值

根因不是服务端没有版本，而是 v1.3.1 状态链在 Controller `/version` 成功后立即返回；Controller 只能给 Core 自身版本，v4.0.1 `/api/status` 中的 `versions.management_panel` / `versions.metacubexd` 根本没有机会进入 UI。

修复后：

- Core 在线状态与 Core 版本仍以 Controller 为第一权威，避免为了版本标签降低实时状态可靠性。
- SwiftUI 以独立 30 秒 cadence 拉取 Management `/api/status`，只做元数据合并，不额外写入流量历史样本。
- GoWebUI 缓存同一份 Management 元数据并合并到 Controller/systemd 快照。
- Management 返回的 stale Core 版本不会覆盖 Controller `/version`。
- 项目升级完成后清理版本元数据缓存并立即刷新，避免升级后仍显示旧面板/XD 版本。

### Bug ②：“Core 控制”右侧“服务信息”卡向上/向下溢出

根因是 SwiftUI 对 `serviceControl` / `serviceInfo` 同时强制外层 `.frame(height: 300)`；右侧 9 行信息的固有高度超过 300pt，SwiftUI 会把超高子视图居中放在固定 frame 中，视觉上就像右卡向上下越界。

修复后两卡使用 `HStack(alignment: .top)`，去掉 300pt 固定外框，内部统一 `minHeight: 328` 并允许内容只向下自然增长。GoWebUI 原本使用 CSS grid stretch，不存在该固定高度问题。
