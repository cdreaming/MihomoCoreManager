...
## v4.0.1 interface parity addendum

- Runtime status stays Controller-first; Management `/api/status` is sampled on a slow cadence to fill Core-panel and MetaCubeXD versions/deployment metadata.
- Reload probes `systemctl show mihomo.service --property=CanReload --value`; the v4.0.1 default unit has no `ExecReload`, so an unsupported systemd reload is skipped and the Management action is used instead.
- Start/restart through systemd best-effort disables the unit first, matching the installer project's no-autostart policy.
- MetaCubeXD open resolution is explicit LAN URL → LAN Management host + reported standalone port → explicit/public URL. It never invents `:29091`.
- Controller `/ui/` / `/Ui/` and known API suffixes can be pasted directly; reverse-proxy prefixes and explicit ports are preserved. `0.0.0.0` / `::` are rejected as client destinations.
