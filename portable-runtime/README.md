...
- 上传/下载状态栏速度由较大值决定共同单位，避免上下两行单位独立跳变。
- 删除固定 53pt 网速块：数字列按两行实际宽度取最大值并右对齐，单位列同起点、右缘贴边；外层不再保留 2pt 左右 padding，元素间距缩为 2pt。
- 状态栏网速本体不显示上传/下载箭头，两行整体略向下；关闭主窗口仍保持状态栏常驻。
- 版本统一为 v1.3.6 / build 1306。
